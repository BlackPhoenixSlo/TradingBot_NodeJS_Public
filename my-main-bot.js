...
console.log('SOL Market Cap:', sol);
    ...
    let key = "_replace_"; //btc eth bybit
...
//   };
    ...
    let webhookUrl = 'https://discord.com/api/webhooks/_replace_/_replace_-sGr_bcV';
...
client_tv = new TradingView.Client({ token: id, signature: certificate });
    webhookUrl = 'https://discord.com/api/webhooks/1193637484354084925/zPQhzT0-_replace_'
   //DisplayBinanceData(client_bybit,webhookUrl,lev);
 await TraderBotS(client,client_tv, client_bybit, weights_no_prov,weights_prov, indicator_id, id, certificate, dbname, markets, timeframes, webhookUrl,lev,NofCoins,leverage,marketCapAndCorrelationData) 
...
await sleep(1500 );
    updateGooglesheets( client,markets, timeframes, type_mybe_provisional,"PGiants2",spreadsheetId);
await sleep(1500 );
    updateGooglesheets( client,markets, timeframes, type_mybe_provisional2,"NoPGiants2",spreadsheetId);
DisplayBinanceData(client_bybit,webhookUrl,lev);
    ...
    client_tv = new TradingView.Client({ token: id, signature: certificate });
    webhookUrl = 'https://discord.com/api/webhooks/_replace_/_replace_-'
   //DisplayBinanceData(client_bybit,webhookUrl,lev);
 await TraderBotS(client,client_tv, client_bybit, weights_no_prov,weights_prov, indicator_id, id, certificate, dbname, markets, timeframes, webhookUrl,lev,NofCoins,leverage,marketCapAndCorrelationData) 
...
await sleep(1500 );
    await updateGooglesheets( client,markets, timeframes, type_mybe_provisional,"PlTopAlts",spreadsheetId);
await sleep(1500 );
    await updateGooglesheets( client,markets, timeframes, type_mybe_provisional2,"NotPTopAlts",spreadsheetId);
DisplayBinanceData(client_bybit,webhookUrl,lev);
  console.log('DONE');
...
//   };
     ...
     webhookUrl = 'https://discord.com/api/webhooks/_replace_/_replace_-sGr_bcV';
